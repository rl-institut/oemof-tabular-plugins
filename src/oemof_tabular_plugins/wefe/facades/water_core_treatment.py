...
class WaterCoreTreatment(MIMO):
    """
    Lumped water core treatment facade based on MIMO.

    Purpose
    -------
    Lumped-parameter facade representing the core (main) treatment stage of a water treatment train, the dissolved-contaminant
    and pathogen removal step, covering technologies such as membrane filtration (MF/UF/NF/RO), electrodialysis, adsorption,
    ion exchange, distillation, advanced oxidation, and biological denitrification. One instance represents whichever single
    core technology has been selected for a given train; the optional brine/biomass/N2 outputs activate only for the technologies
    that actually produce them (e.g. brine for membrane processes, biomass/N2 for biological denitrification, leave both
    unset for a technology like adsorption or UV-based oxidation that produces neither).
    ...
    Notes
    -----
    - Primary flow is water_out_bus [m³/hr]. Capacity constrains the maximum treated water throughput of the core treatment stage.
    - efficiency represents the recovery of whichever core technology this instance models, expect this to vary substantially
      by technology (e.g. ~0.75-0.85 for RO, close to 1.0 for adsorption or UV oxidation).
    - Only set brine_out_bus / waste_biomass_out_bus / N2_gas_bus for
      technologies that physically produce those streams; leave unset
      otherwise so the corresponding conversion factor and cost never enter
      the model.
    """
    ...
    def _apply_flow_parameters(self):
        ...
        # --------------------------------------------------------------
        total_marginal_cost = np.add(self.marginal_cost, self.water_out_variable_costs)
