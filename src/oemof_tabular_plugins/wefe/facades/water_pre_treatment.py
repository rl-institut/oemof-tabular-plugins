...
class WaterPreTreatment(MIMO):
    """
    Lumped water pretreatment facade based on MIMO.

    Purpose
    -------
    Lumped-parameter facade representing the physical/chemical pretreatment stage of a water treatment train: intake, coarse
    and fine screening, grit removal, cartridge filtration, simple oxidation, and coagulation/flocculation, combined into
    one input-output unit. Represents the cumulative volumetric recovery, energy demand, and solids rejection of this multistep
    train, it does not resolve individual unit processes or intermediate water quality. Sits upstream of WaterCoreTreatment,
    protecting it from fouling and clogging by removing coarse solids and grit before the feed reaches membrane- or adsorption
    -based core processes.
    ...
    Notes
    -----
    - Primary flow is water_out_bus [m³/hr]. Capacity constrains the maximum treated water throughput of the pretreatment stage.
    - efficiency represents cumulative volumetric recovery across the train (backwash losses, screenings, grit underflow,
      floc sludge blowdown combined), expect this close to 1.0 for a physical/chemical pretreatment train, unlike membrane
      -based core stages where recovery is a primary design driver.
    """

    # ------------------------------------------------------------------
    # tabular identity
    # ------------------------------------------------------------------
    type: str = "water_pre_treatment"
    name: str = ""
