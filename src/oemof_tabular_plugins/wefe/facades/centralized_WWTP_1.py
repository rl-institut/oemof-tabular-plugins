...
class Centralized_WWTP(Converter, Facade):
    r"""Centralized waste water treatment plant unit with two inputs and two outputs.
    Add parameters to the sludge output stream as and when required later.
    ...
    sludge_out_bus: oemof.solph.Bus
        An oemof bus instance where the unit is connected to with
        its sludge output.
    specific_energy_consumption: float
        Specific electricity demand/consumption in kWh per m³ treated water. Default: 1.0
    efficiency: float
        Share of untreated water that becomes treated water.
        Value between 0 and 1. Default: 0.92
    capacity: numeric
    ...
    specific_energy_consumption: float = 1.0 # kWh/m³
        ...
        self.conversion_factors.update(
            {
                ...
                # Sludge generated per unit of treated water output
                self.sludge_out_bus: sequence((1 / self.efficiency) - 1),
            }
        )
