...
class CompostingToilet(Converter, Facade):
    ...
    compost_out_bus: oemof.solph.Bus
    ...
    bulking_agent_dose: float
        Bulking agent dosage in kg/kg of compost produced. Default: 2.61
    bulking_agent_cost: float
        Bulking agent cost in USD per kg. Default: 0.2
    urine_compost_fraction: float
        Human urine that is required in m³ per kg of compost. Default: 2.17
    feces_compost_fraction: float
        Human feces that is required in kg per kg of compost. Default: 3.91
    leachate_compost_relation: float
        Leachate produced in m³ along with 1 kg of compost. Default: 1.74
    capacity: numeric
    ...
    bulking_agent_dose: float = 2.61 # kg/kg of compost

    bulking_agent_cost: float = 0.2 # USD/kg

    urine_compost_fraction: float = 2.17

    feces_compost_fraction: float = 3.91

    leachate_compost_relation: float = 1.74
    ...
    def build_solph_components(self):
        ...
        self.conversion_factors.update(
            {
                self.human_feces_bus: sequence(self.feces_compost_fraction),
                self.human_urine_bus: sequence(self.urine_compost_fraction),
                self.compost_out_bus: sequence(1),
                self.water_out_bus: sequence(self.leachate_compost_relation),
            }
        )
        ...
        self.outputs[self.compost_out_bus].custom_attributes = {"bulking_agent_dose": self.bulking_agent_dose}
