...
class MIMO_CompostingToilet(MIMO):
    """
    Specialized MIMO-B Composting Toilet facade with code-driven physics.
    ...
    # ---- tabular identity ----
    type: str = "mimo_composting_toilet"
    name: str = ""
    tech: str = "mimo"
    carrier: str = ""
    ...
    compost_out_bus: Bus = None
    water_out_bus: Bus = None

    # ---- physics parameters ----
    feces_compost_fraction: float = 3.91  # kg feces / kg compost
    urine_compost_fraction: float = 2.17  # m³ urine / kg compost
    leachate_compost_relation: float = 1.74  # m³ leachate / kg compost
    feces_density: float = 1060.0          # kg/m³

    # ---- economics ----
    bulking_agent_dose: float = 2.61  # kg bulking agent / kg compost
    bulking_agent_cost: float = 0.2  # USD / kg bulking agent
    marginal_cost: float = 0.0  # USD / kg compost
    ...
    def __init__(self, **attributes):
        ...
        # physics
        # ---------------------------
        self.feces_compost_fraction = attributes.pop(
            "feces_compost_fraction", self.feces_compost_fraction
        )
        self.urine_compost_fraction = attributes.pop(
            "urine_compost_fraction", self.urine_compost_fraction
        )
        self.leachate_compost_relation = attributes.pop(
        ...
        # bulking agent cost per kg compost
        ...
        # ==================================================
        # PHYSICS → CONVERSION FACTORS (normalized to compost output = 1)
        # ==================================================
        ...
        attributes.update(
            {
                ...
                f"conversion_factor_{self.human_feces_bus.label}":
                    sequence(self.feces_compost_fraction),
                f"conversion_factor_{self.human_urine_bus.label}":
                    sequence(self.urine_compost_fraction),

                # outputs
                f"conversion_factor_{self.compost_out_bus.label}":
                    sequence(1.0),
                f"conversion_factor_{self.water_out_bus.label}":
                    sequence(self.leachate_compost_relation),
            }
        )
