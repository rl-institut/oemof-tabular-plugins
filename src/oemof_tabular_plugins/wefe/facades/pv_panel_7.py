...
@dataclass_facade
class PVPanel7(Converter, Facade):
    ...
    def build_solph_components(self):
        ...
        print(geo_params["pitch"])
        print(modules_per_m2)

        # pv efficiency
        pv_efficiency = [
            f.power(rad=gni, t_air=t_air, **pv_params) / gni
            for t_air, gni in zip(t_air_values, gni_values)
        ]
        # capacity efficiency: pv efficiency * how much pv per capacity in kW
        capacity_efficiency = np.array(pv_efficiency) * modules_per_m2 * f.C_W_TO_KW
        # pv nominal output
        pv_values = [
            f.power(rad=ghi, t_air=t_air, **pv_params)
            for t_air, ghi in zip(t_air_values, ghi_values)
        ]

        self.conversion_factors.update(
            {
                self.from_bus: sequence(ghi_to_gni),
